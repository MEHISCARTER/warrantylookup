...
def main(): 
    ...
    menu.add_cascade(label="Enter File", menu=file_menu)
    file_menu.add_command(label="Open", command=open_file)

    #download menu
    download_menu = tk.Menu(menu)
    menu.add_cascade(label="Download", menu=download_menu)
    download_menu.add_command(label="Download CSV", command=download_file)
...
def show_help():
    tk.messagebox.showinfo("How to Use", "1 lah blah")

def show_about():
    tk.messagebox.showinfo("About", "blah blah")
